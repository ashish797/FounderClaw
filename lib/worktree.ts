...
function copyDirSync(src: string, dest: string): void {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    // Skip symlinks to avoid infinite recursion (e.g., .claude/skills/founderclaw → repo root)
...
function getDedupPath(): string {
  return path.join(os.homedir(), '.founderclaw-dev', 'harvests', 'dedup.json');
}
...
export class WorktreeManager {
  ...
  create(testName: string): string {
    ...
    const worktreeBase = path.join(this.repoRoot, '.founderclaw-worktrees', this.runId);
  ...
  harvest(testName: string): HarvestResult | null {
    ...
    try {
      ...
      if (!isDuplicate) {
        // Save patch
        const harvestDir = path.join(os.homedir(), '.founderclaw-dev', 'harvests', this.runId);
  ...
  cleanupAll(): void {
    ...
    const runDir = path.join(this.repoRoot, '.founderclaw-worktrees', this.runId);
    try {
  ...
  pruneStale(): void {
    try {
      ...
      const worktreeBase = path.join(this.repoRoot, '.founderclaw-worktrees');
