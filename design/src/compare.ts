...
<script>
  ...
  function postFeedback(feedback) {
    if (!window.__FOUNDERCLAW_SERVER_URL) return Promise.resolve(null);
    return fetch(window.__FOUNDERCLAW_SERVER_URL + '/api/feedback', {
      method: 'POST',
  ...
  function startProgressPolling() {
    if (!window.__FOUNDERCLAW_SERVER_URL) return;
    var pollCount = 0;
    ...
    var pollInterval = setInterval(function() {
      ...
      fetch(window.__FOUNDERCLAW_SERVER_URL + '/api/progress')
  ...
  function submitRegenerate(detail) {
    ...
    postFeedback(feedback).then(function(result) {
      ...
      } else if (window.__FOUNDERCLAW_SERVER_URL) {
