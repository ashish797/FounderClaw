/**
 * founderclaw design CLI — stateless CLI for AI-powered design generation.
 *
 ...
 *   2. Resolve auth (~/. founderclaw/openai.json → OPENAI_API_KEY → guided setup)
...
function printUsage(): void {
  console.log("founderclaw design — AI-powered UI mockup generation\n");
  console.log("Commands:");
  for (const [name, info] of COMMANDS) {
    console.log(`  ${name.padEnd(12)} ${info.description}`);
    console.log(`  ${"".padEnd(12)} ${info.usage}`);
  }
  console.log("\nAuth: ~/.founderclaw/openai.json or OPENAI_API_KEY env var");
...
async function runSetup(): Promise<void> {
  ...
  if (existing) {
    ...
    saveApiKey(key);
    console.log("Key saved to ~/.founderclaw/openai.json (0600 permissions).");
  }
  ...
  try {
    await generate({
      ...
      output: "/tmp/founderclaw-design-smoke-test.png",
      size: "1024x1024",
      quality: "low",
    });
  ...
  switch (command) {
    case "generate":
      await generate({
        ...
        output: (flags.output as string) || "/tmp/founderclaw-mockup.png",
        check: !!flags.check,
    ...
    case "compare": {
      ...
      const outputPath = (flags.output as string) || "/tmp/founderclaw-design-board.html";
    ...
    case "variants":
      await variants({
        ...
        outputDir: (flags["output-dir"] as string) || "/tmp/founderclaw-variants/",
        size: flags.size as string,
        quality: flags.quality as string,
    ...
    case "iterate":
      await iterate({
        ...
        output: (flags.output as string) || "/tmp/founderclaw-iterate.png",
      });
      break;
    ...
    case "evolve":
      await evolve({
        ...
        brief: flags.brief as string,
        output: (flags.output as string) || "/tmp/founderclaw-evolved.png",
      });
      break;
    ...
    case "gallery":
      gallery({
        ...
        output: (flags.output as string) || "/tmp/founderclaw-design-gallery.html",
      });
      break;
