 ...
 *
 * Resolution order:
 * 1. ~/.founderclaw/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".founderclaw", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.founderclaw/openai.json
  try {
...
/**
 * Save an API key to ~/.founderclaw/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.founderclaw/openai.json: { \"api_key\": \"sk-...\" }");
