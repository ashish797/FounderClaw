 ...
 *
 * Run: OPENAI_API_KEY=$(cat ~/.founderclaw/openai.json | python3 -c "import sys,json;print(json.load(sys.stdin)['api_key'])") bun run design/prototype.ts
 */
...
const API_KEY = process.env.OPENAI_API_KEY
  || JSON.parse(fs.readFileSync(path.join(process.env.HOME!, ".founderclaw/openai.json"), "utf-8")).api_key;

if (!API_KEY) {
  console.error("No API key found. Set OPENAI_API_KEY or save to ~/.founderclaw/openai.json");
  process.exit(1);
}

const OUTPUT_DIR = "/tmp/founderclaw-prototype-" + Date.now();
