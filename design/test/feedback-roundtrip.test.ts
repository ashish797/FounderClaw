...
beforeAll(async () => {
  ...
  // - Injects __FOUNDERCLAW_SERVER_URL into the HTML
    ...
    fetch(req) {
      ...
      if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
        const injected = currentHtml.replace(
          '</head>',
          `<script>window.__FOUNDERCLAW_SERVER_URL = '${url.origin}';</script>\n</head>`
        );
...
describe('Submit: browser click → feedback.json on disk', () => {
  test('clicking Submit writes feedback.json that the agent can poll for', async () => {
    ...
    // Navigate to the board (served with __FOUNDERCLAW_SERVER_URL injected)
    await handleWriteCommand('goto', [baseUrl], bm);

    // Verify __FOUNDERCLAW_SERVER_URL was injected
    const hasServerUrl = await handleReadCommand('js', [
      '!!window.__FOUNDERCLAW_SERVER_URL'
    ], bm);
