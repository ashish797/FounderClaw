...
/** founderclaw's 10 AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES */
...
function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'founderclaw';
  // Don't double-prefix: founderclaw-upgrade → founderclaw-upgrade (not founderclaw-founderclaw-upgrade)
  if (skillDir.startsWith('founderclaw-')) return skillDir;
  return `founderclaw-${skillDir}`;
}
...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string } {
  ...
  if (host === 'codex') {
    ...
    content = content.replace(/~\/\.claude\/skills\/founderclaw/g, ctx.paths.skillRoot);
    content = content.replace(/\.claude\/skills\/founderclaw/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.claude\/skills\/review/g, '.agents/skills/founderclaw/review');
