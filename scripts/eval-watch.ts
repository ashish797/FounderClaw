...
const FOUNDERCLAW_DEV_DIR = path.join(os.homedir(), '.founderclaw-dev');
const HEARTBEAT_PATH = path.join(FOUNDERCLAW_DEV_DIR, 'e2e-live.json');
const PARTIAL_PATH = path.join(FOUNDERCLAW_DEV_DIR, 'evals', '_partial-e2e.json');
...
export function renderDashboard(heartbeat: HeartbeatData | null, partial: PartialData | null): string {
  ...
  if (heartbeat?.runId) {
    const logPath = path.join(FOUNDERCLAW_DEV_DIR, 'e2e-runs', heartbeat.runId, 'progress.log');
