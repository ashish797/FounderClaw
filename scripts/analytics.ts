#!/usr/bin/env bun
/**
 * analytics — CLI for viewing founderclaw skill usage statistics.
 *
...
const ANALYTICS_FILE = path.join(os.homedir(), '.founderclaw', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('founderclaw skill usage analytics');
