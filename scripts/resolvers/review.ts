...
SLUG=$(founderclaw/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
\`\`\`bash
CODEX_PROMPT_FILE=$(mktemp /tmp/founderclaw-codex-oh-XXXXXXXX.txt)
\`\`\`
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(founderclaw/bin/founderclaw-config get codex_reviews 2>/dev/null || true)
...
\`\`\`bash
founderclaw/bin/founderclaw-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
\`\`\`bash
founderclaw/bin/founderclaw-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
**Persist the result:**
\`\`\`bash
founderclaw/bin/founderclaw-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
for PLAN_DIR in "$HOME/.claude/plans" "$HOME/.codex/plans" ".founderclaw/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (\`## FOUNDERCLAW REVIEW REPORT\`)
