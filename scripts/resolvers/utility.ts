...
export function generateSlugEval(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/founderclaw-slug 2>/dev/null)"`;
}

export function generateSlugSetup(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/founderclaw-slug 2>/dev/null)" && mkdir -p ~/.openclaw/ycworld/company/projects/$SLUG`;
}
