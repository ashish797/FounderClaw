...
Check if the diff touches frontend files using \`founderclaw-diff-scope\`:

\`\`\`bash
source <(${ctx.paths.binDir}/founderclaw-diff-scope <base> 2>/dev/null)
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/founderclaw-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
\`\`\`
...
**Local:** \`.founderclaw/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md\`

**Project-scoped:**
\`\`\`bash
eval "$(founderclaw/bin/founderclaw-slug 2>/dev/null)" && mkdir -p ~/.openclaw/ycworld/company/projects/$SLUG
\`\`\`
...
\`\`\`bash
SKETCH_FILE="/tmp/founderclaw-sketch-$(date +%s).html"
\`\`\`
...
$B screenshot /tmp/founderclaw-sketch.png
\`\`\`
...
The screenshot file at \`/tmp/founderclaw-sketch.png\` can be referenced by downstream skills
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/founderclaw-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
Replace STATUS with "clean" or "issues_found", SOURCE with "codex+subagent", "codex-only", "subagent-only", or "unavailable".`;
}

// ─── Design Hard Rules (OpenAI framework + founderclaw slop blacklist) ───
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + founderclaw design methodology.`;
}
...
\`\`\`bash
eval "$(founderclaw/bin/founderclaw-slug 2>/dev/null)"
