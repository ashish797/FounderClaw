...
[ -f .founderclaw/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
\`\`\`
...
If user picks H → write \`.founderclaw/no-test-bootstrap\` and continue without tests.
...
If user picks C → write \`.founderclaw/no-test-bootstrap\`. Tell user: "If you change your mind later, delete \`.founderclaw/no-test-bootstrap\` and re-run." Continue without tests.
