...
URL="$FOUNDERCLAW_SUPABASE_URL"
KEY="$FOUNDERCLAW_SUPABASE_ANON_KEY"
PASS=0
FAIL=0
TOTAL=0
...
check "UPDATE installations"    deny PATCH "installations?installation_id=eq.test_verify_rls" '{"founderclaw_version":"hacked"}'

echo ""
echo "Insert allowed (kept for old client compat):"
check "INSERT telemetry_events" allow POST "telemetry_events" '{"founderclaw_version":"verify_rls_test","os":"test","event_timestamp":"2026-01-01T00:00:00Z","outcome":"test"}'
check "INSERT update_checks"    allow POST "update_checks"    '{"founderclaw_version":"verify_rls_test","os":"test"}'
