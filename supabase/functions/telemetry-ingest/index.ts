// founderclaw telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/founderclaw-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.founderclaw_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        founderclaw_version: String(event.founderclaw_version).slice(0, 20),
      ...
      if (event.installation_id) {
        installationUpserts.set(event.installation_id, {
          version: event.founderclaw_version,
          os: event.os,
        });
      }
    }
        ...
        .upsert(
          {
            ...
            founderclaw_version: data.version,
            os: data.os,
          },
