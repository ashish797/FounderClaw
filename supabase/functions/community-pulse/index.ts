// founderclaw community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes } = await supabase
      .from("crash_clusters")
      .select("error_class, founderclaw_version, total_occurrences, identified_users")
      .limit(5);
    ...
    const { data: versionRows } = await supabase
      .from("telemetry_events")
      .select("founderclaw_version")
      .eq("event_type", "skill_run")
      .gte("event_timestamp", weekAgo)
      .limit(1000);

    for (const row of versionRows ?? []) {
      if (row.founderclaw_version) {
        versionCounts[row.founderclaw_version] = (versionCounts[row.founderclaw_version] ?? 0) + 1;
      }
    }
