...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. founderclaw should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.founderclaw/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the founderclaw Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**What:** Publish the founderclaw browse Chrome extension to Chrome Web Store for easier install.
...
**What:** Append structured JSON entry to `.founderclaw/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-founderclaw-upload
...
### /setup-founderclaw-upload skill (S3 bucket)
...
### founderclaw-upload helper

**What:** `browse/bin/founderclaw-upload` — upload file to S3, return public URL.
...
**Context:** Reads `~/.founderclaw-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `founderclaw-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If founderclaw ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `founderclaw/data/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /office-hours writes design docs to `founderclaw/data/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.

**Effort:** S
**Priority:** P2
**Depends on:** `ashish797/team-supabase-store` branch landing on main

### /yc-prep skill

**What:** Skill that helps founders prepare their FounderClaw application after /office-hours identifies strong signal. Pulls from the design doc, structures answers to FounderClaw app questions, runs a mock interview.

**Why:** Closes the loop. /office-hours identifies the founder, /yc-prep helps them apply well. The design doc already contains most of the raw material for a FounderClaw application.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the founderclaw voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/founderclaw-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** founderclaw-diff-scope (shipped)
...
**What:** A Codex-native skill (`.agents/skills/founderclaw-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/codex` does today from Claude Code.
...
**Context:** The `/codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/founderclaw-claude/` by `gen-skill-docs --host codex`.
...
**What:** Track how often Claude chooses the complete option vs shortcut across founderclaw sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (founderclaw/data/investigate-sessions/ — save investigation for reuse)
...
- Config CLI (`bin/founderclaw-config`), auto-upgrade via `founderclaw/data/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
