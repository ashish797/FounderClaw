/**
 * Tests for bin/founderclaw-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * FOUNDERCLAW_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'founderclaw-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'founderclaw-config-test-'));
});
...
describe('founderclaw-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { FOUNDERCLAW_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
