...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.founderclaw'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.founderclaw', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.founderclaw/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.founderclaw');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.founderclaw', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.founderclaw');
    ...
    test('adds .founderclaw/ to .gitignore if not present', () => {
      ...
      expect(content).toContain('.founderclaw/');
      expect(content).toBe('node_modules/\n.founderclaw/\n');
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('does not duplicate .founderclaw/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.founderclaw/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.founderclaw', 'browse.json') });
    ...
    test('logs warning to browse-server.log on non-ENOENT gitignore error', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      // Create a read-only .gitignore (no .founderclaw/ entry → would try to append)
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
  ...
  describe('getRemoteSlug', () => {
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:ashish797/founderclaw.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('ashish797-founderclaw');
    });

    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/ashish797/founderclaw.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('ashish797-founderclaw');
    });
    ...
    test('parses HTTPS remote URLs without .git suffix', () => {
      const url = 'https://github.com/ashish797/founderclaw';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('ashish797-founderclaw');
    });
