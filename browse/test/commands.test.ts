...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/founderclaw-cli-');
    const execPath = path.join(root, '.claude/skills/founderclaw/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/founderclaw/browse/src/server.ts');
