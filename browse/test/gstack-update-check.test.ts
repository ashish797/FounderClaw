/**
 * Tests for bin/founderclaw-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * FOUNDERCLAW_DIR / FOUNDERCLAW_STATE_DIR / FOUNDERCLAW_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'founderclaw-update-check');

let founderclawDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      FOUNDERCLAW_DIR: founderclawDir,
      FOUNDERCLAW_STATE_DIR: stateDir,
      FOUNDERCLAW_REMOTE_URL: `file://${join(founderclawDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  founderclawDir = mkdtempSync(join(tmpdir(), 'founderclaw-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'founderclaw-state-test-'));
  // Link real founderclaw-config so update_check config check works
  const binDir = join(founderclawDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'founderclaw-config'), join(binDir, 'founderclaw-config'));
});

afterEach(() => {
  rmSync(founderclawDir, { recursive: true, force: true });
...
describe('founderclaw-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '0.4.0\n');
  ...
  test('just-upgraded marker does not mask newer remote version', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(founderclawDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('just-upgraded with no further updates writes UP_TO_DATE cache', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(founderclawDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '0.3.3\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(founderclawDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(founderclawDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(founderclawDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      FOUNDERCLAW_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ FOUNDERCLAW_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(founderclawDir, 'VERSION'), version + '\n');
