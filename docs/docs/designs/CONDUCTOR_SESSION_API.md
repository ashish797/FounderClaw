...
When Claude controls your real browser via CDP (founderclaw `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

founderclaw's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the founderclaw Chrome extension Side Panel that shows:
  ...
  "workspaces": [
    {
      "id": "abc123",
      "name": "founderclaw",
      "branch": "ashish797/chrome-extension-ctrl",
      "directory": "~/founderclaw",
      "pid": 12345,
      "active": true
    }
  ]
...
## What founderclaw builds (extension side)
