# Design: founderclaw Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.founderclaw/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/founderclaw-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/founderclaw-mockup-{timestamp}.png`
...
```typescript
// claude host:
designDir: '~/.claude/skills/founderclaw/design/dist'
// codex host:
designDir: '$FOUNDERCLAW_DESIGN'
```
Note: Codex runtime setup (`setup` script) must also export `FOUNDERCLAW_DESIGN` env var, similar to how `FOUNDERCLAW_BROWSE` is set.

**`$D` resolution bash block** (generated by `{{DESIGN_SETUP}}`):
```bash
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/founderclaw/design/dist/design" ] && D="$_ROOT/.claude/skills/founderclaw/design/dist/design"
[ -z "$D" ] && D=~/.claude/skills/founderclaw/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `FOUNDERCLAW_DESIGN` env var export for Codex host |
...
1. Read `~/.founderclaw/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.founderclaw/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/founderclaw/` install path
...
- Auth module (read ~/.founderclaw/openai.json, fallback to env var, guided setup flow)
...
- Add FOUNDERCLAW_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
