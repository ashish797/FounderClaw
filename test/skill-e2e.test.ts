...
// Eval result collector — accumulates test results, writes to ~/.founderclaw-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.founderclaw', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local founderclaw/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with founderclaw installed globally, the fallback path
    // founderclaw/browse/dist/browse exists, so we get READY.
...
This is a founderclaw issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey founderclaw team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a founderclaw skill. The session preamble detected _SESSIONS=4 (the user has 4 founderclaw windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.founderclaw/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.founderclaw', 'projects', 'test-project');
...
// --- founderclaw-upgrade E2E ---

describeIfSelected('founderclaw-upgrade E2E', ['founderclaw-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'founderclaw-remote-'));
    ...
    // Create mock founderclaw install directory (local-git type)
    const mockFounderClaw = path.join(upgradeDir, '.claude', 'skills', 'founderclaw');
    fs.mkdirSync(mockFounderClaw, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockFounderClaw);
    run('git', ['config', 'user.email', 'test@test.com'], mockFounderClaw);
    run('git', ['config', 'user.name', 'Test'], mockFounderClaw);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockFounderClaw);

    // Write old version files
    fs.writeFileSync(path.join(mockFounderClaw, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockFounderClaw, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockFounderClaw, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockFounderClaw);
    run('git', ['commit', '-m', 'initial'], mockFounderClaw);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockFounderClaw);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockFounderClaw, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockFounderClaw, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockFounderClaw);
    run('git', ['commit', '-m', 'release 0.6.0'], mockFounderClaw);
    run('git', ['push', 'origin', 'HEAD:main'], mockFounderClaw);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockFounderClaw);

    // Copy founderclaw-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'founderclaw-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'founderclaw-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'founderclaw-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('founderclaw-upgrade-happy-path', async () => {
    const mockFounderClaw = path.join(upgradeDir, '.claude', 'skills', 'founderclaw');
    const result = await runSkillTest({
      prompt: `Read founderclaw-upgrade/SKILL.md for the upgrade workflow.

You are running /founderclaw-upgrade standalone. The founderclaw installation is at ./.claude/skills/founderclaw (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/founderclaw — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'founderclaw-upgrade-happy-path',
      runId,
    });

    logCost('/founderclaw-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockFounderClaw, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/founderclaw-upgrade happy path', 'founderclaw-upgrade E2E', result, {
