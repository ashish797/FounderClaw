...
const scriptPath = join(import.meta.dir, "..", "bin", "founderclaw-global-discover.ts");

describe("founderclaw-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/founderclaw-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:ashish797/founderclaw.git");
      const https = normalizeRemoteUrl("https://github.com/ashish797/founderclaw.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/ashish797/founderclaw");
