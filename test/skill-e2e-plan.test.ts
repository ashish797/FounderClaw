...
// Verifies that plan-eng-review writes a "## FOUNDERCLAW REVIEW REPORT" section
...
`);
  ...
  test('/plan-eng-review writes FOUNDERCLAW REVIEW REPORT to plan file', async () => {
    const result = await runSkillTest({
      prompt: `Read plan-eng-review/SKILL.md for the review workflow.
...
CRITICAL REQUIREMENT: plan.md IS the plan file for this review session. After completing your review, you MUST write a "## FOUNDERCLAW REVIEW REPORT" section to the END of plan.md, exactly as described in the "Plan File Review Report" section of SKILL.md. If founderclaw-review-read is not available or returns NO_REVIEWS, write the placeholder table with all four review rows (CEO, Codex, Eng, Design). Use the Edit tool to append to plan.md — do NOT overwrite the existing plan content.
...
This review report at the bottom of the plan is the MOST IMPORTANT deliverable of this test.`,
    ...
    expect(planContent).toContain('## FOUNDERCLAW REVIEW REPORT');

    // Report should be at the bottom of the file
    const reportIndex = planContent.lastIndexOf('## FOUNDERCLAW REVIEW REPORT');
