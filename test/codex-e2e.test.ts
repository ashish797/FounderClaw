...
const CODEX_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'codex-review-findings':   ['review/**', '.agents/skills/founderclaw-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts'],
};
...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install founderclaw-review skill to a temp HOME and ask Codex to list skills
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'founderclaw-review');
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('founderclaw') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);

  // Validates that Codex can invoke the founderclaw-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install founderclaw-review skill and ask Codex to review the worktree
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'founderclaw-review');

    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the founderclaw-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: testWorktree,
      skillName: 'founderclaw-review',
    });
