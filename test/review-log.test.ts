...
function run(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, FOUNDERCLAW_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 10000,
  };
  try {
    const stdout = execSync(`${BIN}/founderclaw-review-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'founderclaw-revlog-'));
  // founderclaw-review-log uses founderclaw-slug which needs a git repo — create the projects dir
...
describe('founderclaw-review-log', () => {
