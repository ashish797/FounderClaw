...
const GEMINI_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'gemini-review-findings': ['review/**', '.agents/skills/founderclaw-review/**', 'test/helpers/gemini-session-runner.ts'],
};
...
describeGemini('Gemini E2E', () => {
  ...
  testIfSelected('gemini-review-findings', async () => {
    // Run founderclaw-review skill via Gemini on worktree (isolated from main working tree)
    const result = await runGeminiSkill({
