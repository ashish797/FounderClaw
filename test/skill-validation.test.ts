...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.founderclaw/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=founderclaw');
  });
  ...
  test('contains Ashish personal plea', () => {
    expect(content).toContain('Ashish, the creator of FounderClaw');
  });
...
describe('Contributor mode preamble structure', () => {
  ...
  for (const skill of skillsWithPreamble) {
    ...
    test(`${skill} uses periodic reflection (not per-command)`, () => {
      ...
      expect(content).not.toContain('After you use founderclaw-provided CLIs');
    });
  }
});
...
describe('Completeness Principle in generated SKILL.md files', () => {
  ...
  test('Completeness Principle includes compression table in tier 2+ skills', () => {
    ...
    expect(content).toContain('CC+founderclaw');
...
// --- founderclaw-slug helper ---

describe('founderclaw-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'founderclaw-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/founderclaw-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for founderclaw-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*founderclaw-slug', '--include=*.tmpl', '--include=founderclaw-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .founderclaw/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('founderclaw-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'founderclaw-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'founderclaw-review', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root founderclaw (browser tool), founderclaw-upgrade (founderclaw-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('founderclaw-') ? skillDir : `founderclaw-${skillDir}`;
  ...
  test('Codex skill names follow founderclaw-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with founderclaw
      expect(dir.startsWith('founderclaw')).toBe(true);
      // Root is just 'founderclaw', others are 'founderclaw-{name}'
      if (dir !== 'founderclaw') {
        expect(dir.startsWith('founderclaw-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('founderclaw-repo-mode');
  });
