...
// Each test gets a fresh temp directory for FOUNDERCLAW_STATE_DIR
let tmpDir: string;

function run(cmd: string, env: Record<string, string> = {}): string {
  return execSync(cmd, {
    cwd: ROOT,
    env: { ...process.env, FOUNDERCLAW_STATE_DIR: tmpDir, FOUNDERCLAW_DIR: ROOT, ...env },
    encoding: 'utf-8',
    timeout: 10000,
  }).trim();
}
...
function setConfig(key: string, value: string) {
  run(`${BIN}/founderclaw-config set ${key} ${value}`);
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'founderclaw-tel-'));
});
...
describe('founderclaw-telemetry-log', () => {
  test('appends valid JSONL when tier=anonymous', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 142 --outcome success --session-id test-123`);
    ...
    expect(events[0].founderclaw_version).toBeTruthy();
  });

  test('produces no output when tier=off', () => {
    setConfig('telemetry', 'off');
    run(`${BIN}/founderclaw-telemetry-log --skill ship --duration 30 --outcome success --session-id test-456`);
  ...
  test('defaults to off for invalid tier value', () => {
    ...
    run(`${BIN}/founderclaw-telemetry-log --skill ship --duration 30 --outcome success --session-id test-789`);
  ...
  test('includes installation_id for community tier', () => {
    ...
    run(`${BIN}/founderclaw-telemetry-log --skill review --duration 100 --outcome success --session-id comm-123`);
  ...
  test('installation_id is null for anonymous tier', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 50 --outcome success --session-id anon-123`);
  ...
  test('includes error_class when provided', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill browse --duration 10 --outcome error --error-class timeout --session-id err-123`);
  ...
  test('handles missing duration gracefully', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --outcome success --session-id nodur-123`);
  ...
  test('supports event_type flag', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --event-type upgrade_prompted --skill "" --outcome success --session-id up-123`);
  ...
  test('includes local-only fields (_repo_slug, _branch)', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 50 --outcome success --session-id local-123`);
  ...
  test('sanitizes skill name with quote injection attempt', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill 'review","injected":"true' --duration 10 --outcome success --session-id inj-1`);
  ...
  test('truncates skill name exceeding 200 chars', () => {
    ...
    run(`${BIN}/founderclaw-telemetry-log --skill '${longSkill}' --duration 10 --outcome success --session-id trunc-1`);
  ...
  test('sanitizes outcome with newline injection attempt', () => {
    ...
    run(`bash -c 'OUTCOME=$(printf "success\\nfake\\":\\"true"); ${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome "$OUTCOME" --session-id inj-2'`);
  ...
  test('sanitizes session_id with backslash-quote injection', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome success --session-id 'id\\\\"","x":"y'`);
  ...
  test('sanitizes error_class with quote injection', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome error --error-class 'timeout","extra":"val' --session-id inj-3`);
  ...
  test('sanitizes failed_step with quote injection', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome error --failed-step 'step1","hacked":"yes' --session-id inj-4`);
  ...
  test('escapes error_message quotes and preserves content', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome error --error-message 'Error: file "test.txt" not found' --session-id inj-5`);
  ...
  test('creates analytics directory if missing', () => {
    ...
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 50 --outcome success --session-id mkdir-123`);
  ...
  test('branch name with quotes does not corrupt JSON', () => {
    ...
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome success --session-id branch-quotes-1`);
  ...
  test('repo slug with special chars does not corrupt JSON', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 10 --outcome success --session-id repo-special-1`);
...
describe('.pending marker', () => {
  test('finalizes stale .pending from another session as outcome:unknown', () => {
    ...
    fs.writeFileSync(
      ...
      '{"skill":"old-skill","ts":"2026-03-18T00:00:00Z","session_id":"old-123","founderclaw_version":"0.6.4"}'
    );

    // Run telemetry-log with a DIFFERENT session — should finalize the old pending marker
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 50 --outcome success --session-id new-456`);
  ...
  test('.pending-SESSION file is removed after finalization', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"stale","ts":"2026-03-18T00:00:00Z","session_id":"stale-session","founderclaw_version":"v"}');
  ...
  test('does not finalize own session pending marker', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"in-flight","ts":"2026-03-18T00:00:00Z","session_id":"same-session","founderclaw_version":"v"}');

    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 50 --outcome success --session-id same-session`);
  ...
  test('tier=off still clears own session pending', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"stale","ts":"2026-03-18T00:00:00Z","session_id":"off-123","founderclaw_version":"v"}');

    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 50 --outcome success --session-id off-123`);
...
describe('founderclaw-analytics', () => {
  test('shows "no data" for empty JSONL', () => {
    const output = run(`${BIN}/founderclaw-analytics`);
    expect(output).toContain('no data');
  });

  test('renders usage dashboard with events', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 120 --outcome success --session-id a-1`);
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 60 --outcome success --session-id a-2`);
    run(`${BIN}/founderclaw-telemetry-log --skill ship --duration 30 --outcome error --error-class timeout --session-id a-3`);

    const output = run(`${BIN}/founderclaw-analytics all`);
  ...
  test('filters by time window', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 60 --outcome success --session-id t-1`);

    const output7d = run(`${BIN}/founderclaw-analytics 7d`);
    expect(output7d).toContain('/qa');
    expect(output7d).toContain('last 7 days');
  });
});

describe('founderclaw-telemetry-sync', () => {
  test('exits silently with no Supabase URL configured', () => {
    // Default: FOUNDERCLAW_SUPABASE_URL is not set → exit 0
    const result = run(`${BIN}/founderclaw-telemetry-sync`);
    expect(result).toBe('');
  });

  test('exits silently with no JSONL file', () => {
    const result = run(`${BIN}/founderclaw-telemetry-sync`, { FOUNDERCLAW_SUPABASE_URL: 'http://localhost:9999' });
    expect(result).toBe('');
  });

  test('does not rename JSONL field names (edge function expects raw names)', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/founderclaw-telemetry-log --skill qa --duration 60 --outcome success --session-id raw-fields-1`);
...
describe('founderclaw-community-dashboard', () => {
  test('shows unconfigured message when no Supabase config available', () => {
    // Use a fake FOUNDERCLAW_DIR with no supabase/config.sh
    const output = run(`${BIN}/founderclaw-community-dashboard`, {
      FOUNDERCLAW_DIR: tmpDir,
      FOUNDERCLAW_SUPABASE_URL: '',
      FOUNDERCLAW_SUPABASE_ANON_KEY: '',
    });
    expect(output).toContain('Supabase not configured');
    expect(output).toContain('founderclaw-analytics');
  });

  test('connects to Supabase when config exists', () => {
    // Use the real FOUNDERCLAW_DIR which has supabase/config.sh
    const output = run(`${BIN}/founderclaw-community-dashboard`);
    expect(output).toContain('founderclaw community dashboard');
