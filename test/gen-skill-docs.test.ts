...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root founderclaw' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('founderclaw_contributor');
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('founderclaw-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('FOUNDERCLAW REVIEW REPORT');
    });
  }
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('FOUNDERCLAW REVIEW REPORT');
    expect(content).toContain('founderclaw-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'founderclaw-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('founderclaw-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('founderclaw-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/founderclaw' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/founderclaw');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('founderclaw')) {
        skills.push({ dir: '.', codexName: 'founderclaw' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('founderclaw-') ? entry.name : `founderclaw-${entry.name}`;
  ...
  test('root founderclaw bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "founderclaw"');
    expect(content).toContain('Use $founderclaw to locate the bundled founderclaw skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('codexSkillName mapping: root is founderclaw, others are founderclaw-{dir}', () => {
    // Root → founderclaw
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw', 'SKILL.md'))).toBe(true);
    // Subdirectories → founderclaw-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw-ship', 'SKILL.md'))).toBe(true);
    // founderclaw-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: founderclaw-founderclaw-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw-founderclaw-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'founderclaw-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
    ...
    expect(frontmatter).toContain('FounderClaw Office Hours');
  });

  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['founderclaw-careful', 'founderclaw-freeze', 'founderclaw-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global founderclaw roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('FOUNDERCLAW_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/founderclaw');
    expect(content).toContain('$FOUNDERCLAW_BIN/founderclaw-config');
    expect(content).toContain('$FOUNDERCLAW_ROOT/founderclaw-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/founderclaw/bin/founderclaw-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/founderclaw/review/ (not founderclaw-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/founderclaw-review
    // but setup puts sidecars under .agents/skills/founderclaw/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use founderclaw/review/ path
    expect(content).toContain('.agents/skills/founderclaw/review/checklist.md');
    expect(content).toContain('.agents/skills/founderclaw/review/design-checklist.md');
    // Wrong: must NOT reference founderclaw-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/founderclaw-review/checklist.md');
    expect(content).not.toContain('.agents/skills/founderclaw-review/design-checklist.md');
  });

  test('sidecar paths in ship skill point to founderclaw/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/founderclaw/review/');
      expect(content).not.toContain('.agents/skills/founderclaw-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/founderclaw/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/founderclaw-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/founderclaw → $FOUNDERCLAW_ROOT
    expect(content).not.toContain('~/.claude/skills/founderclaw');
    expect(content).toContain('$FOUNDERCLAW_ROOT');

    // Rule 2: .claude/skills/founderclaw → .agents/skills/founderclaw
    expect(content).not.toContain('.claude/skills/founderclaw');

    // Rule 3: .claude/skills/review → .agents/skills/founderclaw/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('founderclaw-config') || content.includes('founderclaw-update-check') || content.includes('founderclaw-telemetry-log')) {
        expect(content).toContain('$FOUNDERCLAW_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('founderclaw-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/founderclaw');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of ALL_SKILLS) {
      const content = fs.readFileSync(path.join(ROOT, skill.dir, 'SKILL.md'), 'utf-8');
      expect(content).not.toContain('~/.codex/');
      // founderclaw-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'founderclaw-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'founderclaw-ship', 'SKILL.md'), 'utf-8');
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$FOUNDERCLAW_DIR" "$CODEX_FOUNDERCLAW"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_FOUNDERCLAW_DIR=');
    expect(setupContent).toContain('SOURCE_FOUNDERCLAW_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_FOUNDERCLAW="$INSTALL_FOUNDERCLAW_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_FOUNDERCLAW_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_FOUNDERCLAW_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('founderclaw*');
  });

  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "founderclaw/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "founderclaw/$skill_name"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/founderclaw from being linked as a skill
  test('link_codex_skill_dirs skips the founderclaw sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "founderclaw" ] && continue');
  });

  // T2: Dynamic $FOUNDERCLAW_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic FOUNDERCLAW_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'founderclaw-ship');
    ...
    expect(content).toContain('FOUNDERCLAW_ROOT=');
    expect(content).toContain('$FOUNDERCLAW_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/founderclaw');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('founderclaw/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('founderclaw-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$founderclaw_dir" "$codex_founderclaw"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/founderclaw', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.founderclaw/repos/founderclaw');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies founderclaw- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="founderclaw-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // founderclaw-* dirs should keep their name (e.g., founderclaw-upgrade stays founderclaw-upgrade)
    expect(fnBody).toContain('founderclaw-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only founderclaw-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('founderclaw-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('FOUNDERCLAW_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('founderclaw-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // founderclaw-upgrade is always called founderclaw-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /founderclaw-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'founderclaw-discover-'));
    try {
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('founderclaw-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help founderclaw get better');
    expect(content).toContain('founderclaw-config set telemetry community');
    expect(content).toContain('founderclaw-config set telemetry anonymous');
    expect(content).toContain('founderclaw-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('founderclaw-telemetry-log');
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/founderclaw symlink back to ROOT
