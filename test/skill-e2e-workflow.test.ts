...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'founderclaw-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'founderclaw-ship-work-'));
...
describeIfSelected('founderclaw-upgrade E2E', ['founderclaw-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('founderclaw-upgrade-happy-path', async () => {
    const mockFounderClaw = path.join(upgradeDir, '.claude', 'skills', 'founderclaw');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/founderclaw — use that exact path.`,
    ...
    recordE2E(evalCollector, '/founderclaw-upgrade happy path', 'founderclaw-upgrade E2E', result, {
