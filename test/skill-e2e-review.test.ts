...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // founderclaw-review-read reads from ~/.openclaw/ycworld/company/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock founderclaw-review-read script that returns our test data
    ...
    // Write a mock founderclaw-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'founderclaw-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'founderclaw-review-read'), 0o755);
...
Instead of running founderclaw/bin/founderclaw-review-read, run this mock: ${mockBinDir}/founderclaw-review-read
