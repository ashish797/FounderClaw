...
const HEARTBEAT_PATH = path.join(FOUNDERCLAW_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
...
export async function runSkillTest(options: {
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.founderclaw', 'test-transcripts');
