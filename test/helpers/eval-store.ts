...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.founderclaw-dev', 'evals');

/**
 * Detect project-scoped eval dir via founderclaw-slug.
 * Falls back to legacy ~/.founderclaw-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local founderclaw-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', 'founderclaw/bin/founderclaw-slug 2>/dev/null || founderclaw/bin/founderclaw-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.founderclaw', 'projects', slugMatch[1], 'evals');
