...
5. Write a deploy report skeleton to .founderclaw/deploy-reports/report.md showing the
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
    ...
    const reportDir = path.join(landDir, '.founderclaw', 'deploy-reports');
...
5. Write the dry-run report to .founderclaw/deploy-reports/dry-run-validation.md
...
Just demonstrate the first-run dry-run output.`,
    ...
    const reportDir = path.join(firstRunDir, '.founderclaw', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
(founderclaw-review-read would return NO_REVIEWS).

Simulate what the readiness gate would show:
1. Run founderclaw-review-read equivalent (simulate NO_REVIEWS output)
2. Determine review staleness: Eng Review should be "NOT RUN"
3. Note that Step 3.5a-bis would offer an inline review
4. Write a simulated readiness report to .founderclaw/deploy-reports/readiness-report.md
...
Show what the readiness gate output would look like.`,
    ...
    const reportDir = path.join(reviewDir, '.founderclaw', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
1. Create the .founderclaw/canary-reports/ directory structure
2. Write a simulated baseline.json to .founderclaw/canary-reports/baseline.json with the
...
3. Write a simulated canary report to .founderclaw/canary-reports/canary-report.md following
...
Just create the directory structure and report files showing the correct schema.`,
    ...
    expect(fs.existsSync(path.join(canaryDir, '.founderclaw', 'canary-reports'))).toBe(true);
    const reportDir = path.join(canaryDir, '.founderclaw', 'canary-reports');
...
Instead, demonstrate you understand the workflow:
1. Create the .founderclaw/benchmark-reports/ directory structure including baselines/
2. Write a simulated baseline.json to .founderclaw/benchmark-reports/baselines/baseline.json
...
3. Write a simulated benchmark report to .founderclaw/benchmark-reports/benchmark-report.md
...
Just create the files showing the correct schema and report format.`,
    ...
    expect(fs.existsSync(path.join(benchDir, '.founderclaw', 'benchmark-reports'))).toBe(true);
    const baselineDir = path.join(benchDir, '.founderclaw', 'benchmark-reports', 'baselines');
